	...
	const tpl = `<!doctype html>
				...
				<div class="filter-grid">
					...
					<div class="input-group">
						...
						<select id="scheme" class="input" onchange="applyFilters()">
							...
							{{- range $i, $r := .Results }}
							<option value="{{$r.TargetData.Scheme}}">{{$r.TargetData.Scheme}}</option>
							{{- end }}
						</select>
					</div>
					...
					<div class="input-group">
						...
						<select id="port" class="input" onchange="applyFilters()">
							...
							{{- range $i, $r := .Results }}
							<option value="{{$r.TargetData.Port}}">{{$r.TargetData.Port}}</option>
							{{- end }}
						</select>
					</div>
		...
		function applyFilters(){
			...
			rows.forEach(function(r){
				...
				// Data Attributes 
				// We use normalize() to ensure '80' (string) matches 80 (int) and 'HTTP' matches 'http'
				...
				// Check Matches
				// If the filter is empty (''), it's a match. Otherwise compare normalized values.
